...
import { IMatchingExtensionConfirmed } from 'types';
...
import styles from './ListMatchingExtensionConfirmed.less';

interface Props extends TableProps<IMatchingExtensionConfirmed> {
  onDispatchJob: (name: string) => void;
  dispatching: boolean;
}

const ListMatchingExtensionConfirmed = (props: Props) => {
  const { onDispatchJob, dispatching } = props;
  const columns: Array<ColumnProps<IMatchingExtensionConfirmed>> = [
    {
      title: 'ID',
      dataIndex: 'id',
  ...
  const renderTitle = () => (
    <Row type="flex" justify="space-between" align="middle">
      <Tooltip title="track_matching_extension_confirmed">
        <strong>Matchings Extension Confirmed In Future</strong>
      </Tooltip>
      <Button
        loading={dispatching}
        onClick={() => onDispatchJob && onDispatchJob('track_matching_extension_confirmed')}
      >
        Dispatch Job
      </Button>
    </Row>
...
export default ListMatchingExtensionConfirmed;
