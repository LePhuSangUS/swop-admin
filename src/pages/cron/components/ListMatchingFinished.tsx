...
import { IMatchingExpiry } from 'types';
...
import styles from './ListMatchingFinished.less';

interface Props extends TableProps<IMatchingExpiry> {
  onDispatchJob: (name: string) => void;
  dispatching: boolean;
}

const ListMatchingFinished = (props: Props) => {
  const { onDispatchJob, dispatching } = props;

  const columns: Array<ColumnProps<IMatchingExpiry>> = [
    {
      title: 'ID',
      dataIndex: 'id',
  ...
  const renderTitle = () => (
    <Row type="flex" justify="space-between" align="middle">
      <Tooltip title="track_matching_finished">
        <strong>Matching Finished In Future</strong>
      </Tooltip>
      <Button
        loading={dispatching}
        onClick={() => onDispatchJob && onDispatchJob('track_matching_finished')}
      >
        Dispatch Job
      </Button>
    </Row>
...
export default ListMatchingFinished;
