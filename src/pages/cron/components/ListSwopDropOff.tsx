...
import { ISwopDropOff } from 'types';
...
import styles from './ListSwopDropOff.less';

interface Props extends TableProps<ISwopDropOff> {
  onDispatchJob: (name: string) => void;
  dispatching: boolean;
}

const ListSwopDropOff = (props: Props) => {
  const { onDispatchJob, dispatching } = props;

  const columns: Array<ColumnProps<ISwopDropOff>> = [
    {
      title: 'ID',
      dataIndex: 'id',
  ...
  const renderTitle = () => (
    <Row type="flex" justify="space-between" align="middle">
      <Tooltip title="track_swop_drop_off">
        <strong>Swops Drop Off In Future</strong>
      </Tooltip>
      <Button
        loading={dispatching}
        onClick={() => onDispatchJob && onDispatchJob('track_swop_drop_off')}
      >
        Dispatch Job
      </Button>
    </Row>
...
export default ListSwopDropOff;
