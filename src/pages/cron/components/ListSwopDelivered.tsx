...
import styles from './ListSwopDelivered.less';
...
const ListSwopDelivered = (props: Props) => {
  ...
  const renderTitle = () => (
    <Row type="flex" justify="space-between" align="middle">
      <Tooltip title="track_swop_delivered">
        <strong>Swops Delivered In Future</strong>
      </Tooltip>
      <Button
        loading={dispatching}
        onClick={() => onDispatchJob && onDispatchJob('track_swop_delivered')}
      >
        Dispatch Job
      </Button>
    </Row>
  ...
  return (
    <>
      ...
      <div className={styles.footer}>
        ...
        <div className={styles.footerTag}>
          <span>
            <strong>Note: </strong>Check whether if the shipper delivered end of day pick up{' '}
          </span>
        </div>
        <div>
          <strong>Condition: </strong>
          Swop's status is{' '}
          <Tag color={getSwopStatusColor('shipped')}>{getSwopStatusDisplay('shipped')}</Tag>
        </div>
      </div>
    </>
  );
...
export default ListSwopDelivered;
