...
import { IMatchingRetentionProposed } from 'types';
...
import styles from './ListMatchingRetentionProposed.less';

interface Props extends TableProps<IMatchingRetentionProposed> {
  onDispatchJob: (name: string) => void;
  dispatching: boolean;
}

const ListMatchingRetentionProposed = (props: Props) => {
  const { onDispatchJob, dispatching } = props;
  const columns: Array<ColumnProps<IMatchingRetentionProposed>> = [
    {
      title: 'ID',
      dataIndex: 'id',
  ...
  const renderTitle = () => (
    <Row type="flex" justify="space-between" align="middle">
      <Tooltip title="track_matching_retention_proposed">
        <strong>Matchings Retention Proposed In Future</strong>
      </Tooltip>
      <Button
        loading={dispatching}
        onClick={() => onDispatchJob && onDispatchJob('track_matching_retention_proposed')}
      >
        Dispatch Job
      </Button>
    </Row>
...
export default ListMatchingRetentionProposed;
