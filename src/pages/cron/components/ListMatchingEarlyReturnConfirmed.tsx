...
import { IMatchingEarlyReturnConfirmed } from 'types';
...
import styles from './ListMatchingEarlyReturnConfirmed.less';

interface Props extends TableProps<IMatchingEarlyReturnConfirmed> {
  onDispatchJob: (name: string) => void;
  dispatching: boolean;
}

const ListMatchingEarlyReturnConfirmed = (props: Props) => {
  const { onDispatchJob, dispatching } = props;
  const columns: Array<ColumnProps<IMatchingEarlyReturnConfirmed>> = [
    {
      title: 'ID',
      dataIndex: 'id',
  ...
  const renderTitle = () => (
    <Row type="flex" justify="space-between" align="middle">
      <Tooltip title="track_matching_early_return_confirmed">
        <strong>Matchings Early Return Confirmed In Future</strong>
      </Tooltip>
      <Button
        loading={dispatching}
        onClick={() => onDispatchJob && onDispatchJob('track_matching_early_return_confirmed')}
      >
        Dispatch Job
      </Button>
    </Row>
...
export default ListMatchingEarlyReturnConfirmed;
