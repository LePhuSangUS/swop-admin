...
import { IMatchingExtensionProposed } from 'types';
...
import styles from './ListMatchingExtensionProposed.less';

interface Props extends TableProps<IMatchingExtensionProposed> {
  onDispatchJob: (name: string) => void;
  dispatching: boolean;
}

const ListMatchingExtensionProposed = (props: Props) => {
  const { onDispatchJob, dispatching } = props;
  const columns: Array<ColumnProps<IMatchingExtensionProposed>> = [
    {
      title: 'ID',
      dataIndex: 'id',
  ...
  const renderTitle = () => (
    <Row type="flex" justify="space-between" align="middle">
      <Tooltip title="track_matching_extension_proposed">
        <strong>Matchings Extension Proposed In Future</strong>
      </Tooltip>
      <Button
        loading={dispatching}
        onClick={() => onDispatchJob && onDispatchJob('track_matching_extension_proposed')}
      >
        Dispatch Job
      </Button>
    </Row>
...
export default ListMatchingExtensionProposed;
