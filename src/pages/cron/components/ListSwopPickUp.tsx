...
import styles from './ListSwopPickUp.less';

interface Props extends TableProps<ISwopPickUp> {
  onDispatchJob: (name: string) => void;
  dispatching: boolean;
}

const ListSwopPickUp = (props: Props) => {
  const { onDispatchJob, dispatching } = props;
  const columns: Array<ColumnProps<ISwopPickUp>> = [
    {
      title: 'ID',
      dataIndex: 'id',
  ...
  const renderTitle = () => (
    <Row type="flex" justify="space-between" align="middle">
      <Tooltip title="track_swop_pick_up">
        <strong>Swops Pick Up In Future</strong>
      </Tooltip>
      <Button
        loading={dispatching}
        onClick={() => onDispatchJob && onDispatchJob('track_swop_pick_up')}
      >
        Dispatch Job
      </Button>
    </Row>
