...
import SearchLaundry from 'components/SearchLaundry';
      ...
      >
        <Spin spinning={this.state.loading}>
          <Form layout="horizontal">
            <Form.Item labelAlign="left" label="Laundry" hasFeedback={true} {...formItemLayout}>
              {getFieldDecorator('laundry_id', {
                initialValue: filter?.laundry_id,
              })(
                <SearchLaundry
                  defaultName={filter?.laundry_name}
                  onSelect={(item) => {
                    setFieldsValue({ laundry_id: item.id });
                    this.setState({ laundry_name: item.name });
                  }}
                />,
              )}
            </Form.Item>
            ...
            >
              {getFieldDecorator('time_slot_key', {
                initialValue: filter?.time_slot_value,
                rules: [
                  {
                    required: form.getFieldValue('delivery_date_moment')?.unix() > 0,
